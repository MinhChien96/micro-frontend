...
const LOAN_META = {
  LOAN001: { name: 'Vay tiêu dùng cá nhân', principal: 50_000_000, interestRate: 12.5, termMonths: 36, startDate: '2023-03-05', paidMonths: 19 },
  LOAN002: { name: 'Vay mua nhà ở xã hội',  principal: 800_000_000, interestRate: 8.5,  termMonths: 240, startDate: '2023-09-15', paidMonths: 14 },
};
  ...
  if (!meta) {
    return (
      <div style={{ textAlign: 'center', padding: 48 }}>
        ...
        <button onClick={() => navigate(-1)} style={{ marginTop: 12, padding: '8px 20px', borderRadius: 8, border: '1px solid #e2e8f0', cursor: 'pointer', background: '#fff' }}>← Quay lại</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 13, color: '#64748b' }}>{meta.name} · {meta.interestRate}%/năm · {meta.termMonths} kỳ</div>
        <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>(Hiển thị 12 kỳ đầu tiên)</div>
      </div>

      {/* Table header */}
      <div style={{ display: 'grid', gridTemplateColumns: '60px 100px 1fr 1fr 1fr 1fr 80px', gap: 8, padding: '8px 12px', background: '#f8fafc', borderRadius: 8, marginBottom: 8, fontSize: 11, fontWeight: 700, color: '#64748b' }}>
        <span>Kỳ</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {schedule.map((row) => (
          <div
            key={row.period}
            style={{
              display: 'grid', gridTemplateColumns: '60px 100px 1fr 1fr 1fr 1fr 80px', gap: 8,
              padding: '10px 12px', borderRadius: 8, fontSize: 13,
              background: row.status === 'current' ? '#eff6ff' : row.status === 'paid' ? '#f0fdf4' : '#fff',
          ...
          >
            ...
            <span style={{ textAlign: 'right', fontWeight: 700, color: '#0f172a' }}>{fmt(row.payment)}</span>
