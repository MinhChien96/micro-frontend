...
export default function NewTransfer() {
  ...
  const canNext = () => {
    ...
    if (step === 1)
      return (
        form.recipientName && form.recipientBank && form.recipientAccount && Number(form.amount) > 0
      );
    return true;
  };
    ...
    return (
      <div style={{ maxWidth: 480, margin: '0 auto', textAlign: 'center', paddingTop: 40 }}>
        ...
        <p style={{ color: '#64748b', marginBottom: 8 }}>
          Đã chuyển{' '}
          <strong style={{ color: '#1e3a5f' }}>{fmt(Number(mutation.variables?.amount))}</strong>
        </p>
          ...
          {source && (
            <Card style={{ background: '#f8fafc' }}>
              ...
              <div style={{ fontWeight: 600 }}>
                {source.name} · {fmt(source.balance)}
              </div>
            </Card>
          )}
          {(
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              ...
              >
                ...
                <span
                  style={{ fontWeight: 500, color: '#0f172a', textAlign: 'right', maxWidth: '60%' }}
                >
                  {value}
                </span>
              </div>
