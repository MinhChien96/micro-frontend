...
//   2. REMOTE_HOST_MFE_LOANS[+PORT][+BASE_PATH] — env per môi trường (.env.sit/.uat/...)
//   3. http://localhost:3006/        — dev local
export function resolvePublicPath(): string {
  if (process.env.PUBLIC_URL) return process.env.PUBLIC_URL;
  const host = process.env.REMOTE_HOST_MFE_LOANS;
  if (host) {
    const port = process.env.REMOTE_PORT_MFE_LOANS;
    const basePath = process.env.REMOTE_BASE_PATH_MFE_LOANS;
