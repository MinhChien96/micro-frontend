...
## Quyết định
- Package names scoped `@app/*` (`@app/shell`, `@app/common`, `@app/mfe-*`) — tránh va chạm, dễ rename cho dự án thật (đổi scope `@app` → `@<org>`).
- **MF remote names** (`shell`, `mfe_accounts`…) là JS identifier RIÊNG, độc lập package name → giữ nguyên.
- Brand tập trung ở `@app/common/brand` (`BRAND` const, SSR-safe) — shell/Nav/Helmet/`modern.config` đọc từ đây; đổi 1 chỗ để rebrand.
- Storage keys tập trung ở `@app/common/auth` (`STORAGE_KEYS`, giá trị `app_*`); mọi truy cập qua `getUser/setUser/clearAuth`.

## Rename cho dự án thật
1. Đổi `@app` → `@<org>` trong mọi `package.json` + import specifier + MF `shared:` keys.
2. Sửa `@app/common/brand` (tên/icon/tagline) và `STORAGE_KEYS` nếu muốn.
